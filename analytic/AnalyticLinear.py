...
class RecursiveLinear(AnalyticLinear):
    ...
    @torch.no_grad()
    def fit(self, X: torch.Tensor, Y: torch.Tensor, C: float = 1.0) -> None:
        ...
        # Equation (10) of ACIL
        self.R -= C *self.R @ X.T @ K @ X @ self.R
        # Equation (9) of ACIL
        self.weight += self.R @ X.T @ (Y - X @ self.weight)
        # self.weight -= C * self.R @ X.T @ X @ self.weight
        # self.weight -= C * self.R @ X.T @ X @ self.weight  # 左侧权重减小
        # self.weight += C * self.R @ X.T @ Y  
        # 确保数值稳定性
        if not torch.isfinite(self.weight).all():
            raise RuntimeError("Numerical instability detected. Consider increasing gamma.")
